/**************************************
微信公众号 申工社  签到任务 积分换20块话费
没本事，只能做签到和阅读有赏，其它的等有能力的大佬写吧

*************************
【 签到脚本使用教程 】:
*************************
登录后抓包 fwdt.shengongshe.org域名的Token，填写到sgs,多账号用 @ 分割
...
class UserInfo {
    ...
    async read(){
        try {
            ...
            if(result?.code==200){
                ...
                if(result?.msg=="积分新增失败"){
                    this.readStatus=false;
                    DoubleLog("🔶今日阅读有赏获取积分已达到上限")
                }else{
                    console.log(`🔶${result?.msg}`);
                }   
            }
    ...
    async point(){
        try {
            ...
            let result = await httpRequest(options);
            if(result?.code==200){
                DoubleLog(`✅目前积分为${result?.data?.totalInte}`);
            }else{
                console.log(`🔶${result?.msg}`);  
            }
        } catch (e) {
