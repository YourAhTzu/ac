 ...
 * 项目类型：小程序
 * 项目名称：巧乐兹
 * 项目抓包：抓kapi.yili.com下的accessToken填入变量
 * 项目变量：qlz
 * 项目定时：一天一次
 * cron: 0 9 * * *
 * 搬砖请注明来源
...
let UserCookie = ($.isNode() ? process.env.qlz : $.getdata('qlz')) || '';
let UserCookieArr = [];
let data = '';
let msg =``;
let isSign = false;
let host = 'kapi.yili.com';
...
function sign(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/qlz/api/user/daily/sign?exParams=false`,
            headers: {
                'Host': host,
...
function share(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/qlz/api/integral/product/share?productId=0`,
            headers: {
                'Host': host,
         "Connection": "keep-alive",
        "accessToken": ck[0],
...
function getVersion(timeout = 3 * 1000) {
    return new Promise((resolve) => {
        let url = {
            url: `https://ghproxy.com/https://raw.githubusercontent.com/YourAhTzu/ac/main/qlz.js`,
        }
